...
product_window = Window(
    Format("📦 Товари курсу {dialog_data[selected_course] if 'selected_course' in dialog_data else '❓Не вибрано'}:"),

    ScrollingGroup(
        Select(
            Format("🆔 {item[id]} | {item[name]} - 💰 {item[price]} грн | 📦 {dialog_data[cart][item[id]] if item[id] in dialog_data.get('cart', {}) else 0} шт"),
            items="products",
            id="product_select",
